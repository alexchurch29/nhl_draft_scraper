...
def parse_aging_curves():
    ...
    aging_curves = pd.read_sql_query('''
    ...
    drop_aging_curves_temp = cur.executescript('''
                        DROP TABLE IF EXISTS aging_curves;''')
    aging_curves2.to_sql('aging_curves', conn, if_exists='replace', index=False)
