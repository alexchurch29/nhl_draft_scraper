...
def parse_multi_year(start, end):
    ...
    player_bios = pd.DataFrame.drop_duplicates(player_bios)
    player_stats = pd.DataFrame.drop_duplicates(player_stats)
    goalie_stats = pd.DataFrame.drop_duplicates(goalie_stats)

    conn = sqlite3.connect('nhl_draft.db')
    cur = conn.cursor()

    bios.to_sql('bios', conn, if_exists='replace', index=False)
    player_stats.to_sql('player_stats', conn, if_exists='replace', index=False)

    season_stats = cur.executescript('''
    ...
    career_stats = cur.executescript('''
            create table skater_stats_career as
            select t1.*, round(round(t1.g,2)/t1.gp,2) as G_GP, round(round(t1.a,2)/t1.gp,2) as A_GP, round(round(t1.P,2)/t1.gp,2) as P_GP
            from (select bios.id, league, sum(gp) as GP, sum(g) as G, sum(a) as A, sum(p) as P
            from player_stats
            inner join bios 
            on player_stats.id = bios.id
            group by player_stats.id, league) t1''')
