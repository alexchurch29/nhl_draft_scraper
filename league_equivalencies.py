...
def parse_league_equivalencies():
    """
    Generate league equivalencies for each league based on a weighted average of all paths of len <= 3 from the NHL node
    :return:
    """
    ...
    league_list = ['NHL', 'AHL', 'KHL', 'SHL', 'Liiga', 'NLA', 'DEL', 'Allsvenskan', 'SuperElit', 'Jr. A SM-liiga',
                   'MHL', 'NCAA', 'USHL', 'OHL', 'WHL', 'QMJHL']
    labels = dict()
    for i in range(0, len(league_list)):
    ...
    nx.draw_networkx(G=G, pos=nx.spring_layout(G, k=0.5, iterations=20), node_list=G.nodes(), node_color='orange',
                     node_size=[i * 1000 for i in equivalencies], edge_color='blue', alpha=0.2, arrows=False,
